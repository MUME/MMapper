...
private:
    ...
    {
    private:
        static inline constexpr size_t SIZE = sizeof(uintptr_t);
        static inline constexpr size_t ALIGN = alignof(uintptr_t);
        static_assert(SIZE >= sizeof(UniqueBigSet));
        static_assert(SIZE >= sizeof(size_t));
        static_assert(SIZE >= sizeof(Type)); /* note: must be at least one bit larger */
        alignas(ALIGN) char m_buf[SIZE]{0};
    ...
    public:
        void clear() noexcept
        {
            if (holds_unique_bigset()) {
                ...
                *std::launder(reinterpret_cast<size_t *>(m_buf)) = 0u;
            }
    ...
    public:
        NODISCARD Type get_single_value() const noexcept
        {
            ...
            const auto tmp = *std::launder(reinterpret_cast<const size_t *>(copy.m_buf)) >> 1u;
            return IdHelper_::fromBits(tmp);
        }
        void set_single_value(const Type value) noexcept
        {
            ...
            const auto masked_val = (raw_val & MASK);
            assert(raw_val == masked_val);
            const auto tmp = (masked_val << 1u) | 1u;
            assert(holds_nothing() || holds_single_value());
            *std::launder(reinterpret_cast<size_t *>(m_buf)) = tmp;
            assert(holds_single_value());
