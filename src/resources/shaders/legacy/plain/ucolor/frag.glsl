...
void main()
{
        ...
        if (layerDistance > 0) {
            ...
            // For upper layers: apply organic distortion and organic pattern mask
                ...
                if (layerDistance > maxVisibleLayers) {
                    ...
                    float scale = 0.13;  // Smaller organic patches with sharper edges
                    float threshold = 0.48;  // Adjusted for better balance of visible/transparent areas
                    ...
                    color.a = min(color.a, baseAlpha * holeAlpha);
                }
    ...
    if (uIsNight == 1 && int(vWorldPos.z) == uCurrentLayer) {
        color.rgb *= 0.75;  // 25% darker at night
    }
