...
The `build-appx` CI job was failing on PRs and non-tagged commits due to
...
MakeAppx : error: App manifest validation error: '...0' violates pattern
```

## Root Cause

When `GIT_TAG_COMMIT_HASH` is just a commit hash (e.g., `cd668441`) with no
version info:
